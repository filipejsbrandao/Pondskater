...
class SurfError final : public std::runtime_error {
public:
  int code;
  SurfError(int code_, const std::string& msg)
    : std::runtime_error(msg)
    , code(code_)
  {}
};

[[noreturn]] inline void surf_abort(int code, const std::string& msg) {
  throw SurfError(code, msg);
}

#define SURF_ABORT(code, msg) surf_abort((code), (msg))
