    ...
    for (unsigned i=0; i<3; ++i) {
      if (fit->is_constrained(i)) {
        ...
        if (!input.has_edge(vidxu, vidxv)) {
          ...
          SURF_ABORT(EXIT_INVALID_INPUT, "Invalid input: constraint edge missing in PSLG.");
        }
...
create_bevels_at_vertex(
  ...
  if (bv.degree == 1) {
      if (bv.reflex_beveling_add >= 2) {
        ...
        SURF_ABORT(1, "Beveling: degree-one vertex with >=2 extra vertices not implemented.");
      } {
      assert(l->l()->l == r->l()->l.opposite());
        ...
        if (!compare_NT_real_eq(l->l()->weight, r->l()->weight)) {
          ...
          assert(false);  // Beveling reflex vertex
          SURF_ABORT(1, "Beveling: mismatched incident weights not implemented.");
        }
    ...
    assert(false);  // Beveling reflex vertex
    SURF_ABORT(1, "Beveling: reflex vertices not implemented.");
  }
  edges.push_back(r);
...
set_dcel_vertex(SkeletonDCELHalfedge* start, const Point_2* p, const NT& time) {
  ...
  do {
    ...
    if (!he->next()) {
      ...
      SURF_ABORT(1, "DCEL vertex: NULL next pointer.");
    }
    he = he->next()->opposite();
    degree++;
  } while (he != start);
...
KineticTriangulation::initialize(
  ...
  if (ct.max_component() < restrict_component_) {
    ...
    SURF_ABORT(EXIT_INVALID_INPUT, "Invalid component: out of range.");
  };
  ...
  for (const auto & t: triangles) {
    ...
    if (t.unbounded()) {
      ...
      if ((CGAL::orientation(a, b, c) == CGAL::RIGHT_TURN) != (det < 0)) {
        ...
        SURF_ABORT(EXIT_CGAL_ORIENTATION_MISMATCH, "CGAL orientation mismatch.");
      }
...
handle_event(const Event& event) {
  ...
  if (time <= last_time) {
    ...
    if (count > 10000) {
      ...
      SURF_ABORT(1, "KineticTriangulation: double loop detected.");
    };
  } else {
    count = 0;
    last_time = time;
  ...
  switch (event.type()) {
    ...
    */
    ...
    case CollapseType::NEVER:
      ...
      assert(false);
      SURF_ABORT(1, "Invalid collapse event.");
      break;
    default:
      CANNOTHAPPEN_MSG << "Unexpected event " << event;
      assert(false);
      SURF_ABORT(1, "Unexpected collapse event.");
      break;
  };
