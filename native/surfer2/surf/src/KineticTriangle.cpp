...
if (time_now == last_time) {
  ...
  if (count > 1000) {
    ...
    SURF_ABORT(1, "KineticTriangle: double loop detected (degree 0).");
  };
} else {
  count = 0;
  last_time = time_now;
...
if (time_now == last_time) {
  ...
  if (count > 1000) {
    LOG(ERROR) << "In double loop at line " << __FILE__ << ":" << __LINE__;
    SURF_ABORT(1, "KineticTriangle: double loop detected (degree 1).");
  };
} else {
  count = 0;
  last_time = time_now;
  ...
  if (!could_flip) {
    ...
    switch (result.type()) {
      ...
      default: {
        ...
        assert(false);
        SURF_ABORT(1, "KineticTriangle: unexpected collapse result.");
      }
    }
  }
  ...
  if (determinant_sign == CGAL::NEGATIVE) {
    ...
    switch (CGAL::sign(derivative_at_collapse)) {
      ...
      default:
        ...
        SURF_ABORT(1, "KineticTriangle: invalid derivative sign.");
    }
  }
...
compute_collapse_bounded(const NT& time_now) const { /// {{{
  ...
  switch (num_wavefronts) {
    ...
    default:
      ...
      assert(false);
      SURF_ABORT(1, "KineticTriangle: invalid constrained edge count.");
  }
  ...
  if (is_constrained(idx) && n->is_constrained(nidx)) {
    ...
    if (u->velocity == v->velocity && v->velocity == w->velocity) {
      ...
      if (u->pos_zero == v->pos_zero || v->pos_zero == w->pos_zero) {
        ...
        assert(false);
        SURF_ABORT(1, "KineticTriangle: incident vertices case not implemented.");
      } else {
