...
initialize(const BasicInput& input) {
  ...
  if (input.vertices().size() == 0) {
    ...
    SURF_ABORT(EXIT_INVALID_INPUT, "Invalid input: no vertices.");
  }
  ...
  if (number_of_vertices() != input.vertices().size()) {
    ...
    SURF_ABORT(EXIT_INVALID_INPUT, "Invalid input: duplicate/incident vertices.");
  }
  if (input.edges().size() == 0) {
    LOG(ERROR) << "Invalid input: no edges.";
    SURF_ABORT(EXIT_INVALID_INPUT, "Invalid input: no edges.");
  }
  /* Insert constraints.
   *
  ...
  for (const auto e : input.edges()) {
    if (ct_vertex_handles[e.u] == ct_vertex_handles[e.v]) {
      ...
      SURF_ABORT(EXIT_INVALID_INPUT, "Invalid input: loop edge.");
    }
    ...
    try {
      ...
      SURF_ABORT(EXIT_INVALID_INPUT, "Invalid input: constraint intersection.");
    }
    #else
  ...
  if (dimension() <= 1) {
    ...
    SURF_ABORT(EXIT_INVALID_INPUT, "Invalid input: dimension < 2.");
  }
