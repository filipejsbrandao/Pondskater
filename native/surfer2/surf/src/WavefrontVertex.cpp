...
compute_intersection(const Line_2& a, const Line_2& b) {
  ...
  if (intersect0) {
    if (const Line_2* s = std::get_if<Line_2>(&*intersect0)) {
      return std::make_tuple(LineIntersectionType::ALL, CGAL::ORIGIN);
    } else {
      const Point_2* p = std::get_if<Point_2>(&*intersect0);
      assert(p);
...
make_vertex(
  ...
  switch (lit) {
    ...
    default:
      ...
      assert(false);
      SURF_ABORT(1, "WavefrontVertex: invalid intersection enum.");
  }
...
compute_velocity(
  ...
  if (angle != STRAIGHT) {
    ...
    if (lit != LineIntersectionType::ONE) {
      ...
      assert(false);
      SURF_ABORT(1, "WavefrontVertex: invalid line intersection.");
    }
    ...
    if ( CGAL::orientation(a.l.to_vector(), b.l.to_vector().perpendicular(CGAL::CLOCKWISE)) == CGAL::RIGHT_TURN ) {
      ...
      if (a.normal == b.normal) {
        ...
        assert(false);
        SURF_ABORT(1, "WavefrontVertex: unhandled collinear edge speeds.");
      }
    } else {
      CANNOTHAPPEN_MSG << "This is an infinitely fast vertex.  We should not be in compute_velocity().";
      assert(false);
      SURF_ABORT(1, "WavefrontVertex: invalid infinite speed state.");
    }
  }
  return result;
}
...
std::ostream& operator<<(std::ostream& os, const WavefrontVertex::LineIntersectionType t) {
  ...
  CANNOTHAPPEN_MSG << "Fell through switch which should cover all cases.";
  assert(false);
  SURF_ABORT(1, "WavefrontVertex: invalid intersection type.");
}
